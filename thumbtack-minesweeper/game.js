...
game.js:
  ...
  `new_game()` creates a new game.  When a game has been created, the following
  ...
  For everything else -- UI, mine placement -- I use a 1-dimensional array.
...
function new_game() {
  ...
  validate_game = function() {
    // Check to see if a game is won or lost.
    if (game_over) return false;
...
function get_coordinate_from_position(position) {
  ...
  return new Coordinate(position % BOARD_SIZE, 
...
function get_neighbors(coords) {
  ...
  for (var xi=0; xi < x_vals.length; xi++) {
    ...
    for (var yi=0; yi < y_vals.length; yi++) {
      if (y_vals[yi] < 0 || y_vals[yi] > (BOARD_SIZE - 1) || 
          (y_vals[yi] == coords.y && x_vals[xi] == coords.x)) {
        continue;
      }
